...
check_metrics_file() {
    ...
    if [[ -f "$metrics_file" && $(jq 'has("predxn_runtime_seconds")' "$metrics_file") == "true" && $(jq 'has("scoring_runtime_seconds")' "$metrics_file") == "false" ]]; then
        echo 0  # Return 0 if predxn_runtime_seconds exists and scoring_runtime_seconds does not exist
        ...
        echo 2  # Return 2 if scoring_runtime_seconds exists
    else
        echo 1  # Return 1 if the file doesn't meet the conditions
    fi
}
...
if [[ $(check_metrics_file "$metrics_file") -eq 2 ]]; then
    ...
    # Start the command
    cmd="python score_exp_proprietary.py "
