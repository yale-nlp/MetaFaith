...
from src.metrics.faithfulness import get_faithfulness
...
def run(args):
    ...
    if not args.score_only:    
        ...
        # Score faithfulness -- **Use UNHEDGED input for uncertainty scoring**
        for idx, output in enumerate(results_df["outputs"]):

            print(f"Scoring index {idx}")

            sampled_outputs = sampled_outputs_dict[f"sample_{str(idx)}"]

            faithfulness_score, avg_conf_score, overall_decisiveness_score, assertions_list, conf_responses_list = get_faithfulness(
                answer=output, 
                sampled_answers=sampled_outputs, 
            ) 

            results_df.loc[idx, 'faithfulness'] = faithfulness_score
            results_df.loc[idx, 'confidence'] = avg_conf_score
            results_df.loc[idx, 'decisiveness'] = overall_decisiveness_score
            assertions_dict[f"sample_{str(idx)}"] = assertions_list
            ...
            if idx % 10==0:
                ...
                with open(uncertainty_outputs_file, 'w') as file:
                    json.dump(conf_responses_list, file, indent=4)
